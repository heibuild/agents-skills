...
### 1. Local Storage or GitHub?

Ask the user whether to use local file storage or GitHub issues for the output. If GitHub and the user passes a GitHub issue number or URL as an argument, fetch it with `gh issue view <number>` (with comments). If local file storage, confirm the output directory (default `./docs/issues/`).
